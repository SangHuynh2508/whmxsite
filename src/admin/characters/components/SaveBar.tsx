...
type Props = { dirtyCount: number; status: EditorStatus; message: string; onSave: () => void; onDiscard: () => void; onReload: () => void; hint?: string };

// Bottom strip of the record on desktop, fixed to the screen bottom on phones.
export function SaveBar({ dirtyCount, status, message, onSave, onDiscard, onReload, hint }: Props) {
