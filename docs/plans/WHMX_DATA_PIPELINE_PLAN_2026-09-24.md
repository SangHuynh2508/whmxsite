...
### N2. Release-day runbook (old pipeline, owner-driven) — rehearsed read-only 2026-09-25
1. `cd ../NeoArtifacts && python refresh_masterdata.py` (backs up MasterData, fetches new CFC/lang). Check `MasterData/provenance/launch_*.json` matches the live build.
...
3. `python tools/sync_masterdata_incremental.py --character <NEW_ID> --dry-run`, review, then `--apply` (owner yes). **New characters only** — on already-translated characters it proposes hundreds of spurious rows. It stamps the version from the newest provenance file.
...
5. `python tools/publish_assets.py` (uploads the new card/drawing/archive; `--dry-run` first) → `python tools/build_web_data.py` (also exports `generated_localization.json`).
6. DB (owner yes for each write): `node scripts/import-character-skin.mjs --check` → run → `node scripts/import-character-profile.mjs` (plan) → `--apply`. Must run **before** step 7, otherwise the new character keeps the legacy profile (raw K/T/S/P codes) and is missing from the R2 lore.
7. `node scripts/export-profile-overlay.mjs --shape v2 | python tools/apply_profile_overlay.py` → validators (`validate_data.py`, `validate_skin_roster.py`, `validate_skin_assets.py`, `validate_public_output.py`) → `npm run build`.
8. Owner reviews, then commit → push `main` → `LORE_PUBLISH_PREFIX=lore/production/ node --env-file=.env scripts/publish-lore.mjs`.
...
| P1 | Profile importer | Schema (`character_profiles`, `profile_texts`, `lore_terms`) + `scripts/import-character-profile.mjs` (`--check`). Sources: `characterFiles`, `characterFileTextMap`, `historicalRelicsMap` (incl. timeline), `HistoricalTextMap`, `friendshipDescription`; seed the 195 legacy VI cells once as `legacy_workbook`. Only characters already in the DB. Migration + first import need owner approval (DB writes) ✅ **Status 2026-09-24:** done — migration 0005 applied, first import (133 profiles, 2,260 texts, 139 terms) and 150 legacy VI cells written; re-runs are no-ops. |
| P2 | Resolvers + parity | JS per-entity resolvers (`resolveCharacterProfile(id)`). **Parity gate:** for every character, the resolver's legacy-shape output equals the legacy build's `profile` byte-for-byte before any DB edit; then the new shape (VI, resolved K/T/S, timeline, unlock level) ✅ **Status 2026-09-24:** done — gate 1 byte-identical; gate 2 ok (no raw codes, no department change, 0 published VI); `public/data.json` carries the v2 profile shape. |
| P3 | Publish to R2 | A Vercel Function builds the lore JSON with per-entity resolvers → versioned object on R2 + pointer swap. Triggered about 30 s after an Admin save (debounced) and by an owner button. Frontend overlay loader with CN fallback. No commits/CI (architecture §11, 2026-09-24) ✅ **Status 2026-09-24:** live — lore published to `lore/{development,preview,production}/` in `whmx-assets`, daily private backup in `whmx-backups`, production site reads it. |
...
- 2026-09-24: **Import held: local MasterData is stale.** `NeoArtifacts/MasterData` comes from the 2026-09-15 launch (ConfigVersion `91852aac…`, LangData 5499, ResVersion r1880). The live launch endpoint today returns ConfigVersion `38b3d399…`, LangData 5521, r1897, the same as the MuMu runtime capture `20260917T051530Z_6914dc91` (build_ab r3021 → r3026). Note: the r3026 update did not change config/lang (both sides of the capture already had 38b3d399/5521), so the stale part predates it. Next: owner approves `python refresh_masterdata.py` (backs up MasterData first), then re-run importer `--check`/plan.
- 2026-09-24: Owner approved MasterData refresh: `refresh_masterdata.py` → ConfigVersion `38b3d399…`, LangData 5521, r1897 (backup `MasterData_Backups/MasterData_20260924_163400`). 12 tables changed (BTRuleMap, BattleTowerMap, DungeonBuffMap, QuestionnaireMap, TeamRelayBuffDescMap, activityGroup, avatarFrames, buffMap, dungeonMap, itemMap, langGlobal, questEvent); none of the lore sources. **First profile import applied** (import run `dfc0808b-fca0-4ba8-bd26-e651014846d6`: 133 profiles, 2,260 texts, 139 terms); re-running the plan shows 2,532 unchanged. Legacy seed preview: 150 cells (15 characters × 10), 0 with changed CN; waiting for owner yes. `public/data.json` has not been rebuilt with the new MasterData yet (buffMap/itemMap changes will show up in the next build diff).
- 2026-09-24: Owner approved gate 3: **150 legacy VI cells written** (run `bcf384c9…`; DB: 150 `legacy_workbook`/ok, 2,110 empty, 150 audit rows). **Parity gate 1 passed** (legacy overlay byte-identical to `public/data.json`, first try). **Gate 2 passed** (only `characters.*.profile` differs, no raw codes, no department changes, 0 VI published because legacy VI is never exported). `public/data.json` rewritten with the v2 profiles (+170 KB: timeline, reports kind/unlock, relic terms); 4 validators + `npm run build` pass. The public UI reads only record_id/department/staff_status/entity_status, which are unchanged. Runbook step 5 now includes the overlay. P1 ✅ P2 ✅.
- 2026-09-24: Lore published to R2 `whmx-assets/lore/development/` (`lore.9b1190a97757.json`, 1.1 MB; re-run `unchanged`; pointer 200 with CORS `*`, max-age 60). First backup in private `whmx-backups/backups/lore/development/2026-09-24.json.gz` (874 KB, not reachable publicly — intended); restore dry run against it: no differences. Archive images: upload of 268 PNGs to `whmx-assets/characters/<id>/archives/` approved and running. **Owner decision: do not emit `char.archive` in data.json until the public UI shows it** (no consumer yet); the build change was reverted and saved as a patch at `D:/BaiTapCode/WHMX/_claude_scratch/archive_build_change.patch` (plan Task 12 Step 3 has the same code).
- 2026-09-24: **Final review of the lore branch** (independent reviewer): ready with fixes. Fixed with a failing test first, suite 29/29: (1) restore matched by UUID → now by character ID (backup payload v2; v1 refused; unmatched rows block `--apply`); (2) restore could republish VI over changed CN → forced `source_changed`; (3) `unchanged` trusted the DB row → now compares with the live R2 pointer of that environment (a new `lore/production/` would otherwise be skipped); (4) new CN units on existing profiles did not mark lore unpublished → any import write does; runbook step 5 now has the publish step. Backup in R2 regenerated as v2. **P4 entry requirement:** escape profile fields before rendering (`overviewView.js` uses `innerHTML`; today all values are trusted). Deferred minors: `--repoint` lock/existence check, import plan outside the transaction, REPEATABLE READ for publish, app boot waits ≤5 s for the overlay, duplicated S3 client, no npm test script / `.mts` outside tsc, DB character missing from raw not flagged, restore by date.
- 2026-09-24: **data.json rebuilt with the refreshed MasterData** (owner: fix what is needed first). Of the 12 changed tables only `buffMap` and `itemMap` are read by the build. Result: 7 JSON paths changed, all in A0120 skill `A012003` 喙击再临 (+ its alternate form and Trí Tri upgrade): the game cut `Buff_A0120_10 → Buff_A0120_10_1 → … → Buff_A0139_10` (妙笔), so the one mechanic entry disappears. It was never visible (not in the description, no popup term). D0183/S0181 buff edits and the 3 new items (111, 3016, 5990) do not change the site output (items still 74). Validators pass. `tools/sync_masterdata_incremental.py --dry-run` is **not suitable for already-translated characters**: for A0120/D0183/S0181 it wanted to append 49/85/173 BUFF_STATUS rows plus SKILL/TALENT/PROFILE rows and reported only empty metadata columns (`skill_slot`, `type_label`) as differences; it was not applied. Use it for new characters only. Also noticed: `NeoArtifacts/version.json` still records cfc `91852aac…`/lang 5499 (not updated by the MasterData refresh); the sync tool stamps it into source notes — owner/NeoArtifacts side.
- 2026-09-24: Small-fix batch (owner: fix what is needed first), each with a failing test first, suite 32/32: #10 the public loader no longer waits for the R2 overlay (catalog rendered in ~0.6 s with R2 delayed 4 s; real overlay fetched 200 from `lore/development/` and merged); #6 `--repoint` now runs under the publish lock, refuses a file that is not on R2, and clears `publishedAt`/`publishedByUserId` so Admin keeps showing unpublished changes; #13 profiles/terms that disappear from raw are marked `source_present=false` (reported as `missingFromRaw`), never blanked; real-DB plan still all unchanged. Deferred to P4 (only matter with concurrent Admin writes): #7 import plan outside the transaction, #9 REPEATABLE READ for publish, #8 unpublished-hint semantics. Still deferred: #11 shared S3 client, #14 restore by date, #12 npm test script + `.mts` in tsconfig (both files carry another session's uncommitted edits).
- 2026-09-24: **Vercel Preview (option b).** Owner added 9 Config env vars on Vercel (R2_BACKUP_BUCKET, LORE_PUBLISH_PREFIX, VITE_LORE_POINTER_URL × Production/Preview/Development); Vercel has **no** DB/auth/R2-secret vars yet, so Admin/API only work locally. Pushed the branch; lore published to `lore/preview/` (first publish = `published`, confirming review fix #3). Owner verified the Preview: character page correct, `lore.pointer.json` + `lore.9b1190a97757.json` 200 from `/lore/preview/`. The only console error was `GET /api/admin/session 500` (no auth env on Vercel) → fixed: the session endpoint now returns 200 `{authenticated:false}` when auth is not configured or the visitor is anonymous (test `api/admin/session.test.mjs`); pushed, Preview rebuilt OK. **Before merging to main:** `main` has one commit not on this branch (`e140ade`, V0053 buff attack% fix in `build_web_data.py`); the code merges cleanly, `public/data.json` conflicts → resolve by rebuilding. Merging to main also deploys the whole Admin (Phase 1–3) to production, where login cannot work until the Admin env vars exist on Vercel (P4 decision: which Neon branch for Preview, BETTER_AUTH_ALLOWED_HOSTS).
- 2026-09-24: Owner approved the deploy proposal. **Merged `main` (e140ade, V0053 buff fix) into the branch** (`abbd620`): data.json conflict resolved by a deterministic rebuild + lore overlay; vs the branch only V0053 attack% 1/2/3 → 10/20/30 (equals main) and `raw_paths` provenance of D0080/D0095/V0043/V0059/W0056 (not used by the UI). Validators 4/4, 33/33 tests, `npm run build` OK; pushed; Vercel Preview build success. **Production step not run:** the harness permission guard denied `publish-lore` to `lore/production/` and `git push origin HEAD:main` (classified as production deploy). Waiting for the owner to run them or grant permission. `main` fast-forwards to the branch (no merge needed).
- 2026-09-24: **Production deploy done** (owner granted permission). Lore published to `lore/production/` (`lore.9b1190a97757.json`), then `main` fast-forwarded `e140ade..19e4168`; Vercel production deploy success. Verified on https://whmxsite.vercel.app: `/api/admin/session` → 200 `{authenticated:false}`; data.json has V0053 10/20/30 and A0167 = Liên Minh Hàng Hải; the character page loads `lore.pointer.json` + content 200 from `/lore/production/`, no console errors, no failed requests. Admin on production shows the login screen but cannot sign in until the Admin env vars are added on Vercel (P4). P3 ✅.
- 2026-09-24: Owner noticed production still showed the old admin login. Cause: the Admin Direction B / mobile-dock work (Part C/E) had never been committed. Committed it (`b002630`: React AdminApp/Preview/Accounts, AppNav.tsx, session sign-out event, `vite.config.mts`, `tsconfig.json`, package deps, zod fix; removed the replaced `adminShell.js`, `previewWorkspace.js`, `appNav.js`) and 266 skin avatars that were 404 on production (`291f48d`). The committed tree was built on its own (in-project temp folder, `vite build` OK) before pushing; `main` fast-forwarded to `291f48d`. Production verified: minimal `Đăng nhập` screen, lore from `/lore/production/`, skin avatar 200, no console errors or failed requests. **Still uncommitted (other sessions):** many `tools/*.py` (incl. the release validators, `publish_assets.py`, `sync_masterdata_incremental.py`), docs (state files, plans, architecture/NEXT_STEPS edits), `scripts/import-character-skin.mjs` (roster-count fix), localization batch files, the workbook, root scratch files.
- 2026-09-24: Committed the remaining local work in groups (tools `eb5afd4`, importer N1 fix `2af41d3`, `skin_translation_FINAL_CLEAN.xlsx` `de4fc5e`, docs `3b36b79`); not committed: `docs/admin-redesign/screens/` (account emails; repo is public), `tools/test_fixtures/` (test output), workbook/localization data, root scratch files, agent config folders. Owner-approved fix: A0121 skill A012105 `desc_vi` 'Phong Địch Thanh Dã' → 'Phong Đích Thanh Dã' (镝 = đích). `safe_workbook_mutation` first refused every edit (false positive: SKIN sheet dimension A1:AE418 vs 146 data rows) → fixed to compare last data rows (`016dd89`, tests incl. real-row deletion still rejected); edit applied to exactly 3 cells; data.json/generated_localization.json differ only in those 3 (`0823302`); tools suite 41/41; live on production. Owner decision: 'Thường kích' and 'Đánh thường' may both stay (glossary unification was only for AI translation).
- 2026-09-25: **N2 runbook rehearsed read-only.** OK: `publish_assets --dry-run` (1090 assets, 0 to upload), both importers `--check`, sync `--dry-run`, build/overlay/validators/`npm run build`/lore dry-run (already run 2026-09-24). Fixed: the sync tool stamped the stale `NeoArtifacts/version.json` (cfc 91852aac/lang 5499) into source notes → it now reads the newest `MasterData/provenance/launch_*.json` (38b3d399/5521); version.json left untouched (outside the project). Runbook reordered: DB import of the new character before the lore overlay; publish-lore after push. Not rehearsable without the new character: MuMu runtime-update/character-sync, the real sync `--apply`, translation.
- 2026-09-25: Review #5 (XSS) fixed early, since `overviewView.js` is now committed: profile fields go through `src/lib/escapeHtml.mts` (test + browser proof: an injected `<img onerror>` renders as text). Review #12 done: `npm test` (lore + session + escape tests, 34/34) and `npm run test:tools` (tools unittest); `tsconfig` now typechecks `src/**/*.mts` (tsc clean). Remaining deferred: #7/#9 (with P4), #11 shared S3 client, #14 restore by date. Release-day test happens on the pre-release update (owner, ~1 day before launch).
- 2026-09-25: **P4 brainstorm done.** Owner chose: Lore as a module inside a rebuilt React Khí Giả (Vue replaced), visuals redesigned with huashu (3 directions), motion after UI (CSS/View Transitions first), code with ponytail; Neon option A (development branch becomes production, new development copy; Preview gets Admin on the copy); public lore UI + inline lore edit moved to a separate spec. Spec: `docs/superpowers/specs/2026-09-25-admin-khi-gia-lore-design.md` (awaiting owner review of the file). Also moved `api/admin/session.test.mjs` to `server/` (Vercel counts every api/ file as a function; 4 of 12 used).
- 2026-09-25: Spec approved; phase-1 plan `docs/superpowers/plans/2026-09-25-admin-khi-gia-phase1.md` approved (native, ponytail). **Task 1 step 2 done (owner yes): Neon branches** — `backup-2026-09-25` (br-winter-art-azzb5pld, no compute) from the data branch; old empty `production` renamed `production-old-empty` (br-rough-king…, ep-holy-queen…); data branch renamed **`production`** and set default (br-bold-butterfly-azt1occt, endpoint ep-rapid-dust-azdtb39r unchanged); new **`development`** (br-mute-dawn-aza3tdxi, endpoint ep-blue-dust-azcwulyc) from production. Next: `.env.production.local` + `.env.local` switch (owner pastes the development connection string).
