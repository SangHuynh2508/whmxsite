...
5. `python tools/export_localization_json.py` → `python tools/publish_assets.py` → `python tools/build_web_data.py` → **`node scripts/export-profile-overlay.mjs --shape v2 | python tools/apply_profile_overlay.py`** (lore overlay from the DB, read-only; added 2026-09-24) → validators (`validate_data.py`, `validate_skin_roster.py`, `validate_skin_assets.py`, `validate_public_output.py`) → `node scripts/import-character-skin.mjs --check` (source check for the DB importer, no DB access) → `npm run build`. **Lore (when MasterData changed):** `node scripts/import-character-profile.mjs` (read-only plan) → owner yes → `--apply` → `node --env-file=.env scripts/publish-lore.mjs` (the site overlays R2 lore over data.json, so new CN units only appear after a publish).
