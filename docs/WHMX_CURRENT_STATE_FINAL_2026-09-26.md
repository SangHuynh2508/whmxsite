...
| Tests | `npm test` (91, node:test, <2 s), `npm run test:tools` (Python tools), typecheck `node_modules/.bin/tsc -p tsconfig.json --noEmit`, `npm run build`. Validators: `python tools/validate_{data,public_output,skin_roster,skin_assets}.py` |
...
| Public lore tab "Hồ Sơ Lưu Trữ" | **Shipped to Production 2026-09-26** (plan `superpowers/plans/2026-09-26-public-lore-tab.md`, all 6 tasks; design `public-redesign/lore-tab/direction-approved.md` = B4 museum ticket + book headings). Code `src/features/characters/lore/` (`loreView.mts` pure + tests, `LoreTab.tsx` island), CSS `features/characters/styles/loreTab.css`, `char.archive` in data.json, v2 lore shape carries term descriptions (`relic_info.type|era|museum.detail[_vi]`, `department_detail`). Text reveal on the heading only: `LORE_TAB_REVEAL` in `LoreTab.tsx`. **Term popups need a production lore re-publish** (owner yes) — until then facts show without popups |
| **Extra relic facts (工艺 / 产地 / 出土地 / 有关)** — owner 2026-09-26 ("十二花卉杯 có thêm vài mục") | Not imported. Source `historicalRelicsMap.json` `tag1..tag4` (codes resolve in `HistoricalTextMap` by `Id`: 67 distinct, 0 unresolved; 113 tag values over 134 rows). Evidence: S0132 tag1 `M4012`=五彩 shown as **工艺**, tag3 `H6002`=景德镇官窑 shown as **产地** (owner screenshot). tag2 (`B…`, e.g. 曾侯乙墓) / tag4 (`O…`, e.g. 外销文物) labels **unconfirmed** — `langGlobal` 2314 "出土地在【%s】的文物" / 2317 "和【%s】有关的文物" suggest 出土地 / 有关; confirm with an in-game screenshot of A0061 (tag2) and A0139 (tag4) before labelling. Needs importer + schema (profile columns + lore_terms kinds) + migration (owner yes) | Waiting for owner decision |
...
| **Voice lines (通讯记录)** — owner 2026-09-26: later, on a **separate route** (not the lore tab). Source: MasterData `characterLines.json` (142) + `CharacterLinesLanMap.json` (23 variants); not imported anywhere yet | Not started (later) |
