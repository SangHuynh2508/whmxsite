...
- **Vercel**: Production and Preview each have 10 Secret vars (DB, `BETTER_AUTH_SECRET`, `BETTER_AUTH_ALLOWED_HOSTS`, R2, `R2_MANAGED_ASSET_PREFIX` = `admin-prod` / `admin-dev`) + 9 lore config vars. Preview login works only on the branch alias `whmxsite-git-feat-postgres-admin-crud-siro-da-bao.vercel.app` (behind Vercel SSO). `vercel.json`: `regions: ["sin1"]`, `api/admin/*.js` `maxDuration: 30`. **Never add files under `api/`** (each counts as a function). `.vercelignore` must not exclude anything the server imports (`server/vercelignore.test.mjs`).
...
| Stable-name asset cache headers: `vercel.json` makes all `/assets/(.*)` immutable for a year, but many paths aren't hashed (e.g. `avatars/A0001.png`) — corrected images may not reach returning visitors | Infra debt |
