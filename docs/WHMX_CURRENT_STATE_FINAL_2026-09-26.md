...
| Public lore tab "Hồ Sơ Lưu Trữ" | **Live 2026-09-26** (plan `superpowers/plans/2026-09-26-public-lore-tab.md`; design `public-redesign/lore-tab/direction-approved.md` = B4 museum ticket + book headings). Tab order: … Build · **Hồ Sơ Lưu Trữ** · Thư Viện (gallery always last). Code `src/features/characters/lore/` (`loreView.mts` pure + tests, `LoreTab.tsx` island), CSS `features/characters/styles/loreTab.css`. Shows: quote (recruit line), untranslated notice, intro, relic origin/timeline, reports; ticket = relic name, Loại / Niên đại / Nơi lưu giữ / Kỹ thuật (tag1) / Nơi khai quật (tag2) / Nơi sản xuất (tag3), each opening a description popup (hover brightens the text, footnote under the ticket), stub + serial. **No text-reveal effect** (owner: only on skins). Production lore published `lore.d32aa1ac0d31.json` |
| Relic tag4 (`O…`, e.g. 外销文物 on Trà Diệp Sinh Sản Họa) | Imported (kind `relic_tag`, `structure.relicTags`) but **hidden**: its in-game label is unconfirmed (`langGlobal` 2317 suggests 有关); the owner has no character with tag4 to screenshot. Enable by adding `tag4` to `TAG_LABELS` in `loreView.mts` once confirmed |
...
| **Analytics** | Vercel Web Analytics now records every hash route (`src/app/analytics/pageRoute.mts` → `pageview({route, path})` on hashchange, `inject({disableAutoTrack:true})` in `boot.js`). **Google Analytics 4 is wired but off**: owner creates a GA4 property and gives the Measurement ID (`G-…`); set `VITE_GA_MEASUREMENT_ID` in Vercel (Production + Preview) and redeploy — no code change. GA sets cookies (no consent banner yet) |
