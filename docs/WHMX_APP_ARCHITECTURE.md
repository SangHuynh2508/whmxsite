...
- Keep the Admin shell at `src/admin/layout/AdminApp.tsx` and the Preview workspace in `src/admin/preview/`. Evolve the Vue Character/Skin island only when materially extended; accepted D2.4.1 behavior is a baseline, not a migration trigger.
...
**TARGET (owner, 2026-09-24) — React for new UI, migrated gradually.** New UI, public or Admin, is written in React + TypeScript (`.tsx`), mounted into its own root from the existing boot sequence (precedent: `src/admin/layout/mount.tsx`, `src/app/layout/AppNav.tsx`, the global rail + mobile dock). Existing vanilla-JS views and the Vue Khí Giả island move to React one bounded piece at a time when they are next substantially touched, never as a big-bang rewrite (§13). `tsconfig.json` type-checks every `src/**/*.tsx`.
...
**LOCKED (2026-09-22) — public publication mechanism: Hướng B.** The public site keeps its static `public/data.json` model; routine Admin saves get an automated republish pipeline instead of the current manual `tools/build_web_data.py` run (Great Limbus Library's pattern — no runtime public reads from Postgres). Hướng A (direct hosted/live Postgres reads for public pages) is explicitly not adopted as the mechanism, but the decision is **B without foreclosing A**: the data-shaping/joining logic that currently lives only in `tools/build_web_data.py` must be portable to a live-read path later without a rewrite. Concretely:
- Write the publish pipeline's per-entity shaping as small, named resolver functions (e.g. `resolveCharacterPublicView(id)`, mirroring `WHMX_ENGINEERING_PRINCIPLES.md` §2's worked example), not one monolithic script — a future Hướng-A live-read route would call the same resolver, not duplicate its logic.
- Port that shaping logic to JS/Node, not a Python-subprocess-per-call design — a Python subprocess invoked once at publish time is fine, but nothing in the *live request path* may depend on spawning Python, since that would not survive a later move to on-demand reads.
- The publisher must do an atomic swap of `public/data.json` (no half-published state) and must not run inside a Vercel Function request path (long-running/local or a protected CI export job that produces a reviewable artifact — see `docs/POSTGRES_CRUD_ARCHITECTURE_PROPOSAL_2026-09-19.md` §M for the drafted exporter shape this can build on).
**LOCKED (owner, 2026-09-24) — two publication channels, refining Hướng B.** Still static delivery; the public site never reads Postgres. Details: `docs/plans/WHMX_DATA_PIPELINE_PLAN_2026-09-24.md` §4/§5 (P0 design, section 3).
- **Game data** (numbers, relations, CN source, image URLs) stays in git-tracked `public/data.json`, produced by the local build when the game updates.
- **DB-owned text** (lore first, later names/descriptions) is published as **versioned JSON on R2**: an immutable `…/<domain>.<hash>.json` plus a small pointer file with a short cache. A publish is an atomic pointer swap; rollback means pointing back. It runs automatically about 30 s after an Admin save (debounced), and there is also a manual owner button. No git commit, no redeploy, no CI.
- The publisher may run in a Vercel Function because the publication channel is R2, not the deployment filesystem. The earlier "not inside a Vercel Function request path" rule (whose reason was that Vercel runtime files are not a publication channel) no longer applies to this channel. Per-entity resolvers (`resolveCharacterProfile(id)`) remain the rule.
