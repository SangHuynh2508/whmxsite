...
#### Accelerated Frontend Migration Wave completion — 2026-09-21

- **Calculator (Q1):** only `src/ui/levelProgress.js` moved, to `src/features/calculator/views/levelProgress.js` — it was the sole genuinely Calculator-exclusive module found on audit. `src/data/state.js`, `src/data/calculator.js`, `src/data/loader.js`, `src/data/statResolver.js`, `src/ui/talentGraph.js`, `src/ui/resourceSummary.js`, and `src/ui/calcCharacterPicker.js` were deliberately left in their legacy locations because each is shared with the Character feature (`materialsView.js`, `talentsView.js`, `characterHeader.js`) or the app shell (router/sidebar/bootstrap); moving them would require cross-feature import surgery or helper-splitting out of scope for a path-only batch. `src/ui/dataView.js` is confirmed pre-existing dead code (imported by the router but never invoked; its `#data-view` DOM fallback does not exist in `index.html`) and was left untouched.
- **Weapons (Q2):** `src/ui/weaponsView.js` moved to `src/features/weapons/views/weaponsView.js`. Its CSS was evaluated but not extracted: the `@supports (scrollbar-gutter)` fallback rule groups `.weapons-view`/`.data-view` together with `.character-detail-view`/`.character-catalog-view`/`.main-content` in one rule, and the `.data-page-*`/`.data-card-*` classes are a generic placeholder pattern also referenced by the still-present `dataView.js`; this could not be split cleanly, so it remains in legacy `src/style.css`.
- **Admin Character/Skin workspace ownership (Q3):** `src/characterSkinAdminWorkspace.js` moved to `src/admin/character-skin/characterSkinAdminWorkspace.js`, byte-identical, with the single `adminShell.js` dynamic-import path repaired. No SFC/TS conversion, no internal restructuring, no D2.4.1 behavior change.
- **CSS ownership follow-up (Q4):** the Admin auth/shell/users/Preview-workspace block (`.admin-*` selectors plus the `body.admin-route-active` chrome-hiding rule) was extracted verbatim to `src/admin/styles/adminShell.css`, loaded via a plain `<link>` in `index.html` immediately after `src/style.css` to preserve exact load timing and cascade position. A second candidate block (`.admin-module-nav` / `.character-cms-*`, ~lines 7288–9405 pre-extraction) was investigated but found to interleave today's live selectors with roughly 119 dead rules under a bare `.character-*` prefix left over from a pre-D2.3/D2.4 naming scheme (confirmed dead via repo-wide grep — zero references in current JS), including at least one `@media` block that mixes a dead and a live rule together. This cannot be split by selector family without either duplicating rules or breaking a responsive rule, and deleting the dead rules was out of scope for this batch, so it remains in legacy `src/style.css`, deferred to a dedicated cleanup batch with its own dead-code-removal review.
- All four batches validated with `npm run build`, `git diff --check`, a repo-wide stale-reference search (0 found each time), a case-sensitive import audit, a secret-pattern scan, and browser smoke covering Character Catalog/Detail, Skin Gallery/Detail, Weapons, Calculator (character selection, level change, talent panel, resource/materials recalculation), Admin login shell, and back/forward navigation. `public/data.json` SHA256 (`79a4e8f9…ccd9ac0`) was confirmed unchanged before and after the wave. A local Vercel dev server was not available in this environment, so the authenticated D2 browser proof (`db:d2-local-vercel-proof`) could not run (`ECONNREFUSED :3003`); `db:character-skin-test` and `db:d2-character-skin-proof` both ran directly against the Neon development branch and passed instead.
